...
# --- NOSSOS NOVOS MODULOS ---
import database
import utils
import engine

# --- Configuracao da Pagina (APENAS AQUI) ---
...
# --- MUDANCA: Carrega as horas dinamicas ---
...
if df_analistas.empty:
    ...
    if 'ciclo_anterior' not in st.session_state:
        st.session_state.ciclo_anterior = -1
    if 'df_analistas_editada' not in st.session_state:
        st.session_state.df_analistas_editada = None
    if 'df_rodape_editada' not in st.session_state:
    ...
    if st.session_state.df_analistas_editada is None:
        ...
        # --- MUDANÇA AQUI: Filtra apenas os dias ativos (ativo = 1) ---
        df_dias_ciclo = pd.read_sql_query(
            f"SELECT nome_coluna, data_dia FROM ciclo_dias WHERE id_ciclo = {id_ciclo_selecionado} AND ativo = 1 ORDER BY data_dia ASC",
            conn)
        # --------------------------------------------------------------
        ...
        if not df_historico.empty:
            st.success(
                f"Escala salva encontrada para o ciclo '{ciclos_dict[id_ciclo_selecionado]}'. Carregando do historico.")
            df_escala_pronta = df_historico.pivot(
                index='nome_analista',
                columns='nome_coluna_dia',
                values='turno'
            )
            # Reindex garante que se voce excluiu um dia na config, ele suma da visualizacao do historico tambem
            ...
            if st.button("Gerar Proposta de Escala", type="primary"):
                st.subheader("Debug: Analistas Carregados")
                ...
                with st.spinner(f"Gerando matriz da escala para '{ciclos_dict[id_ciclo_selecionado]}'..."):
                    ...
                    log_container = st.expander("Ver Logs da Geracao", expanded=False)
                    with log_container:
        ...
        if df_escala_pronta is not None:
            ...
            for coluna in df_escala_pronta.columns:
                ...
                if not mentor_encontrado:
                    mentor_encontrado = "(Nao Encontrado)"
                mentor_row.append(mentor_encontrado)
            ...
            st.session_state.df_analistas_editada = df_escala_pronta.drop(index=["MENTOR", "SOBREAVISO"],
                                                                          errors='ignore')
    ...
    if st.session_state.df_analistas_editada is not None:
        ...
        config_colunas = {}
        for nome_coluna in st.session_state.df_analistas_editada.columns:
            config_colunas[nome_coluna] = st.column_config.SelectboxColumn(
                label=nome_coluna,
        ...
        df_para_contar = df_editada_analistas
        turnos_contagem = ["Manha", "Noite", "Integral"]
        contagem_dia = df_para_contar.apply(lambda col: col.value_counts())
        contagem_dia = contagem_dia.reindex(turnos_contagem).fillna(0).astype(int)
        st.dataframe(contagem_dia, use_container_width=True)

        st.divider()

        # 2. Tabela de Carga Horaria
        st.subheader("Carga Horaria por Analista")

        contagem_analista = df_para_contar.apply(lambda row: row.value_counts(), axis=1).fillna(0)
        for t in ["Manha", "Noite", "Integral"]:
            if t not in contagem_analista.columns: contagem_analista[t] = 0

        # --- USO DAS HORAS DINAMICAS ---
        ...
        def highlight_risk(val):
            color = '#ff4b4b' if val > 30 else ''
            return f'background-color: {color}'


        df_exibicao = contagem_analista[["Manha", "Noite", "Integral", "Horas_Decimal"]].copy()
        df_exibicao[["Manha", "Noite", "Integral"]] = df_exibicao[["Manha", "Noite", "Integral"]].astype(int)

        st.dataframe(
            df_exibicao.style
            .applymap(highlight_risk, subset=['Horas_Decimal'])
            .format({'Horas_Decimal': format_hours}),
            use_container_width=True
        )
        ...
        df_final_para_salvar = pd.concat([
            df_editada_analistas,
            df_editada_rodape
        ])
        ...
        with col1_save:
            if st.button("Salvar no Historico", type="primary"):
                try:
                    ...
                    # --- CORREÇÃO DO ERRO ---
                    # Forçamos um nome para o indice antes de resetar.
                    # Isso garante que a coluna criada se chamara 'nome_analista_temp' e nao 'index' ou outra coisa
                    ...
                    df_para_salvar.to_sql('escala_salva', conn, if_exists='append', index=False)
                    conn.commit()
